...
popt_all = np.ndarray((6, 198, 512), dtype=np.float32)
# for ispatial in range(sza_list[ifile].shape[0]):
popt_all[:] = np.nan
for ispatial in range(0, 512):
    print(ispatial)
    ...
    for iwav in range(198):
        ...
        wvl_files = np.array([])
        popt_files = np.array([])
        for ifile in range(len(sza_list)): # read multiple files, specific wavelength and spatial point
...
# # ax.plot_trisurf(sza_files, vza_files, diff_files, cmap='plasma')
                ax.scatter(sza_files, vza_files, diff_files, cmap='plasma', s=1)
                ax.set_xlabel('SZA')
                ax.set_ylabel('VZA')
...
# f = FortranFile('sample.unf', 'w')
# f.write_record(popt_all)
# f.close()
# quit()
