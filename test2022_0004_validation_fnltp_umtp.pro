...
gems_o3p = ds_read_gems_l2_o3p(fn)

gems_o3p_lat = gems_o3p.latitude
gems_o3p_lon = gems_o3p.longitude

nanidx = where(gems_o3p_lat lt -1e29, /null)
gems_o3p_lat[nanidx] = !values.f_nan

nanidx = where(gems_o3p_lon lt -1e29, /null)
gems_o3p_lon[nanidx] = !values.f_nan
...
gemsnum = n_elements(gems_o3p_lat)
...
for ipix = 0, gemsnum-1 do BEGIN
  ...
  if finite(x) eq 1 and finite(y) eq 1 then begin
    ...
    if n_elements(GemsPixonFNLidx) gt 0 and GemsPixonFNLidx ne -999 then begin
      ...
      gems_tp_on_fnl_grid[gemspixonFNLidx] =  gems_o3p.TropopausePressure[ipix]
