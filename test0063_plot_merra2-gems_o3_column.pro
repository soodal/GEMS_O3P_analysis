...
for ix=0, sz[1]-1 do begin
  ...
  for iy=0, sz[2]-1 do begin
    if total(finite(pl[ix, iy, *])) ne 0 then begin
      ...
      trco[ix, iy] = total(o3prfs[ix,iy,ip+1:nl-1],/nan,3)
...
gems_to3 = reform(gemsl2o3p.columnAmountO3[*, *, 2])
plot_sat_proj, gems_to3 - tco, lon, lat, $
  title='GEMS O3P Under 300 hPa - MERRA2 O3 Under 300 hPa', $
  cb_title='O3[DU]', $
  colortable=72, $
  /ctreverse, $
  range=[-20, 20], $
  pngfile='./plot/merra2_collocated/20210621/gemso3p_310340_FNL_ML-merra_o3_totalozone_20210621.png';, /scp_send
...
plot_sat_proj, gems_to3 - tco, lon, lat, $
  title='GEMS O3P TotalO3 hPa - MERRA2 Total O3', $
  cb_title='O3[DU]', $
  colortable=72, $
  /ctreverse, $
  range=[-20, 20], $
  pngfile='./plot/merra2_collocated/20210621/gemso3p_310340_FNL_ML-merra_o3_totalozone_20210621_ecf02.png';, /scp_send
end
