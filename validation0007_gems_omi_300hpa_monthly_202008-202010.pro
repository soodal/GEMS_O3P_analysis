...
for imon = 8, 10 do begin
  jd_list = timegen(start=julday(imon, 1, 2020, 00, 45), $
                    final=julday(imon+1, 1, 2020, 23, 45)-1, units='HOURS')
  monthly_omivals = []
  monthly_gemsvals = []
  flag_dailyplot = 0
  ...
  for itime = 0, n_elements(jd_list)-1 do begin 
    ...
    endif else if year eq 2021 and imon gt 6 then begin
      gemso3p_outfilelist = file_search($
...
if n_elements(total_gemsvals) gt 1 and $
  ...
  plot_gems_validation, total_omivals, total_gemsvals, $
    filename='./plot/gems_l2_o3p_val_with_omi_' + $
      '202008-202010_under300hpa_ecf0.2.png', $
    ytitle='GEMS Tropospheric O3', $
    xtitle='OMI Tropospheric O3', $
    cblim=[0, 2000], $
    range=[0, 60], $
