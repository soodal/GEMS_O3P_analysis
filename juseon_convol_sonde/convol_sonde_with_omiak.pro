PRO calc_o3cum ,use_spline, pres0, cum0, Ptop, Pbot, out
   ...
   if pres(0) gt pres(1) then begin
      pres = reverse(pres) & cum = reverse(cum) 
   endif

   out = !values.f_nan
   
   if ptop ge pbot then return
   if ptop lt min(pres) or pbot gt max(pres) then return
...
;   avgk     ; OMI avgk
;   ntp      ; OMI tropopause pressure level if ntp eq -1 then define tropopause with insert_ptrp
;
; output 
...
; insert_ptrp; if present as 500 hPa, omisco is for partial column between 500 hPa to sontop
;                                     omitco is for partial column between bottom to 500 hPa
; midpres    ; if present as 100 hPa, omisco200 is for partial column between 100hpa to sontop
;                          omitco200 is for partial column between bottom to 100hpa

pro convol_sonde_with_omiak, sonpres0, sono30, omipres, apozprof, ozprof, $
  avgk, ntp, sonstlvl, sondu, sonsco, sontco, sonsco200, sontco200, omisco, $
...
sonsco    = missing ; omipres(sonstlvl) 에서 omipres(ntp) 까지
sontco    = missing ; son(nsl) 에서 omipres(ntp)까지
sonsco200 = missing ; omipres(sonstlvl) 에서 200 까지
sontco200 = missing ; son(nsl)에서 200까지
omitco    = missing
omitco200    = missing
omiatco200 = missing
omiatco   = missing
omisco    = missing ; omipres(nl) 에서 200까지
omisco200 = missing ; omipres(sonstlvl) 에서 omipres(ntp) 까지
omiasco    = missing
omiasco200 = missing
sondu0 = fltarr(nl)  & sondu0(*) = missing  ; original sonde on OMI grid
sondu  = sondu0                             ; convolved sonde 
