pro plot_fitweights, filename, xidx, yidx
...
data = ds_read_gems_l2_o3p(filename, $
  ...
  'SimulatedRadiances', 'FitWeights', 'WavelengthsWholeRange'])
...
;for in = 0, n_elements(selected_idx)-1 do begin 
  ...
  ;IF mm[0] LE -0.3 THEN BEGIN
    ;yrange[0] = mm[0]
  ;ENDIF
  p1 = plot(wav[*, xidx, yidx],fitweights[0:fitresdims[0]-1], /buffer, $
    xtitle='Wavelength[nm]', $
    ytitle='FitWeights', $
    title='GEMS L2 O3P ' + fit_range + 'nm Fit Weights', $
    symbol='s', $
    ;yrange=yrange, $
    xrange=[300, 340], $
    ;name='\c Residuals', $
  ...
  pngfile = './plot/' +basename + '_fitweights_x' +sXidx +'_y' +sYidx +'.png'
