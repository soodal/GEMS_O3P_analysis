...
;if file_test(datafile) then begin
  ...
  ; GEMS O3P EOSRL
  ;=============================================================================
  search_str = '/data/private/soodal/EOSRL/EOSRL/GK2_GEMS_L2_O3P_' + $
    datetime_str + '_wli300_ML_prec000000_maxit10_ozminmax2x_BIN4x4.nc'

  fl = file_search(search_str)
  eosrl_data = ds_read_gems_l2_o3p(fl[-1])

  eosrl_lat = eosrl_data.latitude
  eosrl_lon = eosrl_data.longitude
  ...
  ; GEMS O3P Synthetic
  ;=============================================================================
  ;search_str = '/data/private/soodal/softcal_test/corrected/model_300_340/GK2_GEMS_L2_O3P_' + $
    ;datetime_str + '_winliminit300_ML_prec000000_maxit10_ozminmax2x_BIN4x4.nc'
  search_str = '/data/private/soodal/softcal_test/corrected/model_300_340/GK2_GEMS_L2_O3P_20210329_0345_winliminit300_prec000000_climML_b4x4_o20211129T184747Z_clima_maxiter10_ecf0.nc'
  ...
  irr3d_nier = fltarr(simrad_sz[0], simrad_sz[1], simrad_sz[2])

  for i=0, simrad_sz[0]-1 do begin
    irr3d_nier[i, *, 0:simrad_sz[2]-1] = nier_irr
  endfor
  ...
  for ix=0,simrad_sz[0]-1 do begin
    for iw=0, simrad_sz[2]-1 do begin
      nier_gems_fitspec[ix, *, iw] = nier_rad[ix, *, iw]/irr3d_nier[ix, *, iw]*nier_gems_rad_norm[ix, *]/nier_gems_irrad_norm[*]
    endfor
  endfor
  ;----------------------------------------------------------------------------
  ...
  irr3d_eosrl = fltarr(simrad_sz[0], simrad_sz[1], simrad_sz[2])

  for i=0, simrad_sz[0]-1 do begin
    irr3d_eosrl[i, *, 0:simrad_sz[2]-1] = eosrl_irr
  endfor
  ...
  for ix=0,simrad_sz[0]-1 do begin
    for iw=0, simrad_sz[2]-1 do begin
      eosrl_gems_fitspec[ix, *, iw] = eosrl_rad[ix, *, iw]/irr3d_eosrl[ix, *, iw];*eosrl_gems_rad_norm[ix, *]/eosrl_gems_irrad_norm[*]
    endfor
  endfor
  ...
  save, filename=datafile
endif else begin
  restore, datafile
endelse

; ===========================================================================
; PLOT profile for Cities
; ===========================================================================
...
if plot_cities then begin
  for icity=0, n_elements(cities_lat) -1 do begin
    ...
    if pixidx ne -999 then begin 
        ...
        p2.save, outputpath + '300340_synthetic_x' + string(pixindices[0], format='(i03)') +  $
            '_y' + string(pixindices[1], format='(i03)') + '_' + $
          cities_name[icity] + '.png'
        p2.close
        ...
        plot_softcal_rad, syntheticwav, l1cradwav, $
          ...
          pixindices[0], pixindices[1], cldstr, suffix='_300340_adjusted'
...
for icf = 0.05, icf_final, 0.05 do begin
  ...
  if n_elements(idx) ge 1 then begin
    ...
    for ip = 0, n_elements(idx)-1 do begin
      ...
      if pixindices[0] eq 91 and $
          (pixindices[1] eq 93 or $
          pixindices[1] eq 123 or $
          pixindices[1] eq 179 or $
          pixindices[1] eq 326 or $
          pixindices[1] eq 376 or $
          pixindices[1] eq 423) then begin
    ...
    ;outputpath = outputpath + 'point_profile/'
        ...
        p2.save, outputpath + 'profile_synthetic_300340_x' + string(pixindices[0], format='(i03)') +  $
...
xx = [30, 30, 30, 30, 30, 30]
xx = [91, 91, 91, 91, 91, 91]
yy = [80, 180, 280, 380, 480, 500]
yy = [93, 123, 179, 326, 376, 423]
...
yrange = [0, 0.25]
i = 0
edgelons[0] = lon[xx[i], yy[i]]
edgelons[1] = lon[xx[i]-1, yy[i]]

edgelats[0] = lat[xx[i], yy[i]]
edgelats[1] = lat[xx[i]-1, yy[i]]

get_omler_specific_time_loc, omler, month, day, edgelons, edgelats, alb
albstr = string(alb, format='(f04.2)')
cldstr = string(ecf[xx[i], yy[i]], format='(f4.2)')
latstr = string(eosrl_lat[xx[i], yy[i]], format='(f5.2)')
lonstr = string(eosrl_lon[xx[i], yy[i]], format='(f5.2)')
p1 = plot(syntheticwav[xx[i], *], eosrl_gems_fitspec[xx[i], yy[i], *], /buffer, $
  axis_style=1, $
  xtitle='Wavelength[nm]', $
  ytitle='Radiance[W/cm^2/cm/sr]', $
  title='GEMS EOSRL Normalized Radiance', $
  ;symbol='s', $
  yrange=yrange, $
  xrange=[300, 340], $
  name='ALB='+albstr + ' lat=' + latstr, $
  color=colors[*, i], $
  position=pos, $
  overplot=i)
p1_1 = plot(syntheticwav[xx[i], *], synthetic_fitspec[xx[i], yy[i], *], /buffer, $
  axis_style=1, $
  ;xtitle='Wavelength[nm]', $
  ;ytitle='Radiance[W/cm^2/cm/sr]', $
  ;title='GEMS EOSRL Normalized Radiance', $
  ...
  ;xrange=[300, 340], $
  linestyle=1, $
  name='synthetic ALB='+albstr + ' lat=' + latstr, $
  color=colors[*, i], $
  ;position=pos, $
  overplot=1)
p1['axis1'].color='black'
...
latstr = string(eosrl_lat[xx[i], yy[i]], format='(f5.2)')
p2 = plot($
  syntheticwav[ypos, *], eosrl_gems_fitspec[xx[i], yy[i], *], $
  /buffer, $
  name='ALB='+albstr + ' lat=' + latstr, $
  ;position=pos, $
  color=colors[*, i], $
  /overplot)
p2_1 = plot($
  syntheticwav[ypos, *], synthetic_fitspec[xx[i], yy[i], *], $
  /buffer, $
  linestyle=1, $
  name='synthetic ALB='+albstr + ' lat=' + latstr, $
  ;position=pos, $
...
yaxis2 = axis('Y', LOCATION='right', yrange=yrange, TARGET=p1, $
  color='black', $
  title='')
xaxis2 = axis('X', LOCATION='TOP', xrange=p1.xrange, TARGET=p1, $
  color='black', $
  tickname=['', '', '', '', ''])

leg = legend(target=[p1, p2, p3, p4, p5, p6])

p1.save, './plot/EOSRL/eosrl_fitspec_6points.png'
p1.close

;default_fitspec = eosrl_gems_fitspec[xx[5], yy[5], *]
default_fitspec = synthetic_fitspec[xx[5], yy[5], *]

yrange1 = [-0.1, 0.1];yrange - yrange[1]/2.
i = 0
edgelons[0] = lon[xx[i], yy[i]]
edgelons[1] = lon[xx[i]-1, yy[i]]
...
latstr = string(eosrl_lat[xx[i], yy[i]], format='(f5.2)')
lonstr = string(eosrl_lon[xx[i], yy[i]], format='(f5.2)')
p1 = plot(syntheticwav[xx[i], *], eosrl_gems_fitspec[xx[i], yy[i], *]-default_fitspec, /buffer, $
  axis_style=1, $
  xtitle='Wavelength[nm]', $
  ytitle='Radiance[W/cm^2/cm/sr]', $
  title='GEMS EOSRL Normalized Radiance', $
  ...
  overplot=i)
p1['axis1'].color='black'
p1_1 = plot(syntheticwav[xx[i], *], synthetic_fitspec[xx[i], yy[i], *]-default_fitspec, /buffer, $
  ;axis_style=1, $
  ;xtitle='Wavelength[nm]', $
  ;ytitle='Radiance[W/cm^2/cm/sr]', $
  ;title='GEMS EOSRL Normalized Radiance', $
...
latstr = string(eosrl_lat[xx[i], yy[i]], format='(f5.2)')
p2 = plot($
  syntheticwav[ypos, *], eosrl_gems_fitspec[xx[i], yy[i], *]-default_fitspec, $
  /buffer, $
  name='ALB='+albstr + ' lat=' + latstr, $
  ;position=pos, $
  color=colors[*, i], $
...
yaxis1 = axis('Y', LOCATION='left', yrange=yrange1, TARGET=p1, $
  color='black', title='')
xaxis1 = axis('X', LOCATION='BOTTOM', xrange=p1.xrange, TARGET=p1, $
  color='black', title='');, $
  ;tickname=['', '', '', '', ''])

yaxis2 = axis('Y', LOCATION='right', yrange=yrange1, TARGET=p1, $
  color='black', $
  title='')
xaxis2 = axis('X', LOCATION='TOP', xrange=p1.xrange, TARGET=p1, $
  color='black', $
...
p1.save, './plot/EOSRL/eosrl_fitspec_6points_diff.png'
p1.close
