...
x_para = xval 
y_para = yval 
X_name = 'AERONET_AOD [443nm]'
Y_name = 'GEMS_AOD [443nm]'
...
for j = minv, maxv, delta  do begin
  ...
  ;; delta 구간내에 있는 data 갯수 찾기
    idx_histo= where(yval ge j $
      and yval lt j+delta $
      and xval ge i $
      and xval lt i+delta, idx_histo_num)
    ...
    if idx_histo_num gt 0.0 then begin
      ...
      polyfill, xbox, ybox, $
        color = bytscl(idx_histo_num, min=0.0 ,max=5.0, top=254)
    endif
  endfor
endfor
