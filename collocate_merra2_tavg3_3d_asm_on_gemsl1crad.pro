pro collocate_merra2_tavg3_3d_asm_on_gemsl1crad, year, month, day, hour, minute $
  , method=method
...
gemsfp = '/data2/tmp/L1C/1015/binning/'
gemsfn = 'GK2_GEMS_L1C_' $
...
fl = file_search(gemsfp+gemsfn)
print, fl

if file_test(fl[0]) and file_test(merra2fn) then begin
  gemsl1crad = ds_read_gems_l1c_4x4(fl[0])
  ...
  gems_sz = size(gemsl1crad.pixel_longitude, /dim)
    ...
    for ilon = 0, gems_sz[0]-1 do begin
      for ilat = 0, gems_sz[1]-1 do begin
        ...
        merra2_nearest_index = search_closest_pixel( $
          ...
          gemsl1crad.pixel_longitude[ilon, ilat], gemsl1crad.pixel_latitude[ilon, ilat], $
          maxlimit=0.5)
  ...
  gemstime_jday = ds_gemsl2o3p_time2julday(gemsl1crad.image_acquisition_time)
  ...
  outncfn = 'merra2_tavg3_3d_asm_collocated_on_gemsl1crad_'+yyyy+mm+dd+'_'+hh+mi+'.nc4'
  ...
  ncdf_varput, id, _lon, gemsl1crad.pixel_longitude
  ncdf_varput, id, _lat, gemsl1crad.pixel_latitude
  ncdf_close, id
