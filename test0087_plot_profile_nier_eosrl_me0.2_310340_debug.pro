...
;if file_test(datafile) then begin
  ...
  for ix = 0, simrad_sz[0]-1 do begin
      for iy = 0, simrad_sz[1]-1 do begin
          eosrl_rad[ix, iy, 0:simrad_sz[2]-1] = 1.6*eosrl_gemsl1crad.image_pixel_values[ix, iy, eosrl_startidxarr[ix, iy]:eosrl_startidxarr[ix, iy]+simrad_sz[2]-1]
      endfor
  endfor
...
p2.save, outputpath + 'profile_me0.5_310340_x' + string(pixindices[0], format='(i03)') +  $
    '_y' + string(pixindices[1], format='(i03)')  + '_debug_fitspec1.6.png'
p2.close
...
indices = [91, 376]
  ...
  if not file_test(outfiletest) then begin
    file_mkdir, outfilepath
  endif
  pngfile = outfilepath + 'gems_merra2_synthetic_simrad_me0.5_adj_sig_ecf' + $
    '_x' + string(indices[0], format='(i03)') + $
    '_y' + string(indices[1], format='(i03)') + '_debug_fitspec1.6.png'
