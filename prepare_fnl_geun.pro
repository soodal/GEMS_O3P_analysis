...
PRO read_fnl_nc, date, utc, fnl, fnl_new, dir, get_rearray=get_rearray
  ...
  if nsel1 eq 1 then begin
     ...
     if dd eq 0 then begin
        dd = enday(yy,mm-1) & mm = mm - 1
     endif
     if mm eq 0 then begin
        mm = 12 & yy = yy - 1
     endif
     adate = string(yy, format='(i4.4)') + string(mm, format='(i2.2)')+ string(dd, format='(i2.2)')
     afile = findfile(dir +'fnl_'+adate + '_18_*.nc', count=nfile) & if nfile ne 4 then stop
     file  = [afile(0), file]
  endif

  yy = fix(strmid(date, 0, 4)) &  mm = fix(strmid(date, 4, 2)) & dd = fix(strmid(date, 6, 2))

  if nsel2 eq 1 then begin
        dd = dd+1
     if dd gt enday(yy,mm) then begin
        dd = 1 & mm = mm + 1
     endif
     if mm gt 12 then begin
        mm = 1 & yy = yy + 1
        dir   = '/home/Data/FNL/'+string(yy,format='(i4.4)') + '/'
     endif
     adate = string(yy, format='(i4.4)') + string(mm, format='(i2.2)')+ string(dd, format='(i2.2)')
