...
for imon = 8, 10 do begin
  ...
  hours = timegen(start=julday(imon, 1, 2020, 00, 45), $
    final=julday(imon+1, 0, 2020, 06, 45), $
    units='H')
  ...
  ;for ifile=0, n_elements(gemso3p_outfilelist)-1 do begin
    ...
    gemso3p_hourfilelist = file_search($
      '/data2/L2_GEMS/val_1008/GK2_GEMS_O3P_2020' $
      + mm[i] + dd[i]+'_' +hh[i]+ mi[i]+'*.nc4', $
      count=nfile)
    ...
    if nfile ne 0 then begin
      ...
      omivals = []
      gemsvals = []
      collocate_gemsl2o3p_omil2profoz, year[i], mon[i], day[i],$
        hour[i], minute[i], gemsvals, omivals, $
