...
merra2_o3prfs = fltarr(sz[1], sz[2], nl)
...
FOR ih=0,nl-1 DO BEGIN
  dh = h1[*, *, ih] - h1[*, *, ih+1]
  merra2_o3prfs[*,*,ih] =  number_o3[*,*,ih]/nair*dh * m2mm * mm2du
ENDFOR

merra2_o3prfs[where(merra2_o3prfs GE 1000 or merra2_o3prfs lt 0)] = !values.f_nan
merra2_to3 = total(merra2_o3prfs[*, *, 0:nl-1],/nan,3)

merra2_trco = fltarr(sz[1], sz[2])
merra2_sco = fltarr(sz[1], sz[2])
for ix=0, sz[1]-1 do begin
  ...
  for iy=0, sz[2]-1 do begin
    if total(finite(pl[ix, iy, *])) ne 0 then begin
      ...
      merra2_trco[ix, iy] = total(merra2_o3prfs[ix,iy,ip+1:nl-1],/nan,3)

      merra2_sco[ix, iy] = total(merra2_o3prfs[ix, iy,0:ip],/nan,3)
      ;print, ix, iy, merra2_trco[ix, iy,
    endif
  endfor
endfor
...
plot_sat_proj, merra2_trco, lon, lat, $
...
plot_sat_proj, merra2_to3, lon, lat, $
...
;plot_sat_proj, merra2_sco, lon2d, lat2d, $
...
gemsl2cld = ds_read_gems_l2_o3p(gemscldfn)

;ds_gems_l2o3p_accum, gemsl2o3p, gemso3accum, hpa=300
gems_to3 = reform(gemsl2o3p.columnAmountO3[*, *, 0])
plot_sat_proj, gems_to3 - merra2_to3, lon, lat, $
  title='GEMS O3P(MERRA2 Synthetic) Total O3 - MERRA2 Total O3', $
  cb_title='O3[DU]', $
  colortable=72, $
  /ctreverse, $
  range=[-30, 30], $
  pngfile='./plot/merra2_collocated/20210621/gemso3p_merra2synthetic_300340-merra2_totalo3_20210621.png';, /scp_send
...
plot_sat_proj, gems_to3 - merra2_to3, lon, lat, $
  ...
  /ctreverse, $
  range=[-30, 30], $
  pngfile='./plot/merra2_collocated/20210621/gemso3p_merra2synthetic_300340-merra2_totalo3_20210621_ecf02.png';, /scp_send
end
