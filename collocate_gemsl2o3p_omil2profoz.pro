...
if not keyword_set(savepath) then begin
  savepath = '/home/soodal/data/collocate_gemsl2o3p_profoz/'
endif
...
if not isSaved then BEGIN
  ...
  if gemsfn ne !null then begin
    ...
    for ifile=0, nfiles-1 do begin
      ...
      if fi.size gt 1500 then begin
        ...
        roiidx=WHERE(profozresult.longitude GE limit[1] AND $
                  ...
                  profozresult.latitude LE limit[2], ntmp, /null); and $
        ...
        if ntmp ge 1 then begin 
          ...
          nanidx = where(omio3ap lt -1.e29, /null)
    ...
    omionroinum = n_elements(omitimes)

    omiRoiIdx = lonarr(omionroinum)
    omiRoiIdx[*] = -999

    yn_omi_cross_time = intarr(omionroinum)
    yn_omi_cross_time[*] = 0

    ; loop for every collocation point in a scene for the GEMS
    for ipix = 0, omionroinum-1 do BEGIN
      x = omilons[ipix]
      y = omilats[ipix]

      if finite(x) eq 1  and finite(y) eq 1 then begin
        OmiPixOnGemsIdx = search_closest_pixel(gemslon, gemslat, x, y, maxlimit=0.5)
        if n_elements(OmiPixOnGemsIdx) gt 0 and OmiPixOnGemsIdx ne -999 then begin
          OmiPixOnGemsIndices = array_indices(gemslon, OmiPixOnGemsIdx)
          omiRoiIdx[ipix] = OmiPixOnGemsIdx
          omi_o3p_on_gems_grid[OmiPixOnGemsIndices[0], OmiPixOnGemsIndices[1], *] =  omio3s[*, ipix]

          omi_o3p_accum_on_gems_grid[OmiPixOnGemsIndices[0], OmiPixOnGemsIndices[1]] = omio3accums[ipix]
        endif
      ENDIF

      ; time difference check
      if omiRoiIdx[ipix] ge 0 then begin
        if abs(omijulday[ipix] - gemsjulday[omiRoiIdx[ipix]]) lt 1./24./2. then BEGIN
  ...
  nanidx = where(gems_ecf lt -1e29 , /null)
  gems_ecf[nanidx] = !values.f_nan
  gems_idx = []
  gems_altitude = gemsvars.Altitude
  gems_pressure = gemsvars.Pressure

  colgemsidx = where(omiRoiIdx ge 0, /null)
  ; OmiOnGemsIndices has different origin with gemsindices.
  if n_elements(colgemsidx) ge 1 then begin
    OmiOnGemsIndices = array_indices(gemslon, omiRoiIdx[colgemsidx])
  endif
  colgemsidx_sz = size(colgemsidx, /dim)

  for ipix=0, colgemsidx_sz[0]-1 do BEGIN
    if omiRoiIdx[colgemsidx[ipix]] GE 0 and yn_omi_cross_time[colgemsidx[ipix]] eq 1 then begin
      ...
      if omiszas[colgemsidx[ipix]] lt 70. $
          ...
          ;and omiecf[colgemsidx[ipix]] le 0.2 $
            and gems_ecf[omiRoiIdx[colgemsidx[ipix]]] le 0.2 $
            and gems_ecf[omiRoiIdx[colgemsidx[ipix]]] gt 0 $
            and gems_sza[omiRoiIdx[colgemsidx[ipix]]] lt 70. $
            ;and abs(gems_sza[omiRoiIdx[ix, iy]] - omivars.SolarZenithAngle[ix, iy]) lt 10 $
            then begin
        ...
        gems_o3s = [[gems_o3s] , $
          [reform(gems_o3[OmiOnGemsIndices[0, ipix], $
            OmiOnGemsIndices[1, ipix], *])]]
        gems_o3aps = [[gems_o3aps] , $
          [reform(gems_o3ap[OmiOnGemsIndices[0, ipix], $
            OmiOnGemsIndices[1, ipix], *])]]

        gems_idx = [gems_idx, omiRoiIdx[colgemsidx[ipix]]]
        gems_alts = [[gems_alts], $
          [reform(gems_altitude[OmiOnGemsIndices[0, ipix], $
            OmiOnGemsIndices[1, ipix], *])]]
        gems_press = [[gems_press], $
          [reform(gems_pressure[OmiOnGemsIndices[0, ipix], $
            OmiOnGemsIndices[1, ipix], *])]]

        gems_effectivecloudfraction = [gems_effectivecloudfraction, $
          gems_ecf[omiRoiIdx[colgemsidx[ipix]]]]

        gems_solarzenithangle = [gems_solarzenithangle, $
          gems_sza[omiRoiIdx[colgemsidx[ipix]]]]

        gems_latitude = [gems_latitude, $
          gemslat[omiRoiIdx[colgemsidx[ipix]]]]

        gems_longitude = [gems_longitude, $
          gemslon[omiRoiIdx[colgemsidx[ipix]]]]
  ...
  ;if n_elements(where(omiRoiIdx ge 0 and $
