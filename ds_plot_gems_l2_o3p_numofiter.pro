pro ds_plot_gems_l2_o3p_numofiter, gemsl2fn, outputpath = outputpath, scppath = scppath
...
numofiter = data.NumberOfIterations
...
numofiter_geolocidx = where(lon gt 0 and lat gt 0 and o3strato gt 0, /null)
...
mag = bytscl(numofiter[o3total_geolocidx], min=0, max=max(numofiter[o3total_geolocidx])
p1 = scatterplot(lon[o3total_geolocidx], lat[o3total_geolocidx], $
  ...
  symbol='dot', $
  sym_size=0.7, $
  title='Number of Iterations for 310-340 nm fitting', $
  axis_style=0)
...
pngfile = outputpath + basename + '_numofiter.png'
