...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
MODULE Share_l2_o3p_mod_write_read_Synt
...
!-------------------------------------------------------------------------------
      ...
      USE Share_MOD_Constants_Variables_Synt, ONLY:  gci_nline_l2,       &
                                                gci_nxtrack2_l2,    &
                                                gci_nlayer,         &
                                                gci_nlayerp,        &
                                                gci_nwavel3_l2,     &
    ...
    !----------O3P----------!
     ! <O3Profile>
      ...
      TYPE :: o3p_g_fld
       ...
       INTEGER(KIND=4) , POINTER    :: Line           (:,:)     !Line                ! add 2016.09.23 
       INTEGER(KIND=4) , POINTER    :: Pix            (:,:)     !Pix                 ! add 2016.09.23                 
...
!
      ...
      hdferr = GEMS_Share_Hdf5WriteData(O3Pds%l2_o3p_file_path, O3Pds%file_path_sz, grp_path, O3Pds%grp_path_sz, &
                                        data_name, O3Pds%data_name_sz, L2O3P%o3p_gfld%Line, pstart, pedge)
      IF (hdferr == 0) THEN
            CALL GEMS_Share_MOD_log(llvl, L2O3P%o3p_gfld%Line, nline,nxtrack2_o3p, data_name)
      ELSE
            GOTO 2999
      ENDIF
      ...
      hdferr = GEMS_Share_Hdf5WriteData(O3Pds%l2_o3p_file_path, O3Pds%file_path_sz, grp_path, O3Pds%grp_path_sz, &
                                        data_name, O3Pds%data_name_sz, L2O3P%o3p_gfld%Pix, pstart, pedge)
      IF (hdferr == 0) THEN
            CALL GEMS_Share_MOD_log(llvl, L2O3P%o3p_gfld%Pix, nline,nxtrack2_o3p, data_name)
      ELSE
            GOTO 2999
      ENDIF
...
!      grp_path  = O3Pds%o3p_ggrp_path
      ...
      !-----------------  Line       ---------------
      IF (ASSOCIATED(L2O3P%o3p_gfld%Line)) DEALLOCATE(L2O3P%o3p_gfld%Line)
      data_name = O3Pds%Line_DataName
      ALLOCATE (L2O3P%o3p_gfld%Line(nxtrack2_o3p,nline), STAT = ierr)
           IF(ierr .NE. 0) THEN
              ctl_msg=data_name
              GOTO 999
           ENDIF
      ...
      !-----------------  Pix        ---------------
      IF (ASSOCIATED(L2O3P%o3p_gfld%Pix)) DEALLOCATE(L2O3P%o3p_gfld%Pix)
      data_name = O3Pds%Pix_DataName
      ALLOCATE (L2O3P%o3p_gfld%Pix(nxtrack2_o3p,nline), STAT = ierr)
           IF(ierr .NE. 0) THEN
              ctl_msg=data_name
              GOTO 999
           ENDIF
...
!--------------------------------
      ...
      !-----------------  Line       ---------------
      IF (ASSOCIATED(gds_L2O3P%o3p_gfld%Line)) DEALLOCATE(gds_L2O3P%o3p_gfld%Line)
      ALLOCATE (gds_L2O3P%o3p_gfld%Line(nxtrack2_o3p,nline), STAT = ierr)
           IF(ierr .NE. 0) THEN
              ctl_msg="Line"
              GOTO 999
           ENDIF
      ...
      !-----------------  Pix        ---------------
      IF (ASSOCIATED(gds_L2O3P%o3p_gfld%Pix)) DEALLOCATE(gds_L2O3P%o3p_gfld%Pix)
      ALLOCATE (gds_L2O3P%o3p_gfld%Pix(nxtrack2_o3p,nline), STAT = ierr)
           IF(ierr .NE. 0) THEN
              ctl_msg="Pix"
              GOTO 999
           ENDIF
...
!--------------------------------
      ...
      !-----------------  Line  ---------------
      gds_L2O3P%o3p_gfld%Line = DEF_iVAL

      !-----------------  Pix  ---------------
      gds_L2O3P%o3p_gfld%Pix = DEF_iVAL
...
!--------------------------------
      ...
      !-----------------  Line       ---------------
      IF (ASSOCIATED(local_L2O3P%o3p_gfld%Line)) DEALLOCATE(local_L2O3P%o3p_gfld%Line)
      ALLOCATE (local_L2O3P%o3p_gfld%Line(nxtrack2_o3p,nline), STAT = ierr)
           IF(ierr .NE. 0) THEN
              ctl_msg="Line"
              GOTO 999
           ENDIF
      ...
      !-----------------  Pix        ---------------
      IF (ASSOCIATED(local_L2O3P%o3p_gfld%Pix)) DEALLOCATE(local_L2O3P%o3p_gfld%Pix)
      ALLOCATE (local_L2O3P%o3p_gfld%Pix(nxtrack2_o3p,nline), STAT = ierr)
           IF(ierr .NE. 0) THEN
              ctl_msg="Pix"
              GOTO 999
           ENDIF
...
!--------------------------------
      ...
      !-----------------  Line  ---------------
      local_L2O3P%o3p_gfld%Line = DEF_iVAL

      !-----------------  Pix  ---------------
      local_L2O3P%o3p_gfld%Pix = DEF_iVAL
...
END MODULE Share_l2_o3p_mod_write_read_Synt
