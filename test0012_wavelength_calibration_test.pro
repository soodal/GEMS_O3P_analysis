...
scp_dest = 'soodal@164.125.38.179:/home/soodal/WindowsHome/Pictures/scp/' + $
  'wavelength_calibration_test/' + fitrange + '_' + L1Cmaker
...
latclr_idx = where(lat ge 20 and lat lt 40 $
  and lon ge 120 and lon lt 130 $
  and ecf lt 0.2)
...
measured = fitres + simrad

npix = n_elements(latclr_idx)
; pick 10 pixel from 1/20, 3/20, 5/20, ..., 19/20  from the whole idx
selected_idx = floor(npix / 2)

residuals = fitres[*, latclr_idx[selected_idx]]$
  /simrad[*, latclr_idx[selected_idx]] * 100
...
;p2 = plot(wav[0:fitresdims[0]-1], stdres, 'r', /buffer, $
  ;/over, name='Stddev')
p3 = plot(wav[0:fitresdims[0]-1], measured[*, selected_idx], 'b', /buffer, $
  /current, $
  axis_style=0, $
  name='Measured', $
  position=pos )

p4 = plot(wav[0:fitresdims[0]-1], simrad[*, selected_idx], 'r', /buffer, $
  /over, $
  axis_style=0, $
  name='simulated', $
  position=pos )

yaxis3 = axis('Y', LOCATION='right', yrange=p3.yrange, TARGET=p3, $
    color='BLUE')
xaxis3 = axis('X', LOCATION='right', yrange=p3.yrange, TARGET=p3, $
    color='BLUE')

;yaxis4 = axis('Y', LOCATION='right', yrange=p4.yrange, TARGET=p4, $
    ;color='RED')
;xaxis4 = axis('X', LOCATION='right', yrange=p4.yrange, TARGET=p4, $
    ;color='RED')
...
pngfile = './plot/fitting_residual_' + date + '_' + fitrange $
  + '_20gelatlt40_' + strtrim(string(latclr_idx[selected_idx]),2) $
  + '_softcal.png'
...
measured_sidx = min(where(finite(measured[*, selected_idx]), /null))
measured_fidx = max(where(finite(measured[*, selected_idx]), /null))

simrad_sidx = min(where(finite(simrad[*, selected_idx]), /null))
simrad_fidx = max(where(finite(simrad[*, selected_idx]), /null))

wav_sidx = min(where(finite(wav), /null))
wav_fidx = max(where(finite(wav), /null))

wav_hr_range = wav[wav_fidx] - wav[wav_sidx] 
nwav_hr = ceil(wav_hr_range*100)
wav_hr = wav[wav_sidx] + findgen(nwav_hr)/100.

measured_hr = interpol(measured[selected_idx, $
  wav_sidx:wav_fidx], wav[wav_sidx:wav_fidx], wav_hr)
