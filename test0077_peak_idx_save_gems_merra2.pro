...
if not file_test(savefn) then begin
  ...
  nanidx = where(rad lt -1e29, /null)
  rad[nanidx] = !values.f_nan

	sz = size(rad, /dimension)

	gems_rad_norm = total(rad, 3)/total(finite(rad))
  ...
  nanidx = where(irr lt -990, /null)
  irr[nanidx] = !values.f_nan

	gems_irrad_norm = total(irr, 2)/total(finite(irr))
	for iw=0, 203 do begin 
...
for iy = 0, sz[1]-1 do begin
  ...
  ;for ix = 130, 130 do begin 
    ...
    print, 'gems:', gems_local_extrema_index[gems_huggins_extrema_idx]
    print, 'merra2:', merra2_local_extrema_index[merra2_huggins_extrema_idx]
    ...
    if initialize eq 0 then begin
      ...
      gems_start_index_array = fltarr(sz[0], sz[1], gems_extrema_num)
      initialize = 1
    endif
    ...
    for iex=0, merra2_extrema_num-1 do begin 
      ...
      gems_start_index_array[xidx, yidx, iex] = gems_start_index
